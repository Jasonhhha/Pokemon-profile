...
const badges = [
  ...
  { id: 'projects', name: '灵感研究员', description: '探访作品研究所', icon: Sparkles },
...
export default function App() {
  ...
  return <div className={`app ${night ? 'is-night' : ''}`}>
    ...
    <main>
      ...
      <div className="world-layout">
        <aside className="profile-sidebar">
          ...
          <button className="contact-button" onClick={() => open('contact')}><Mail size={16} /><span>来打个招呼</span><ArrowUpRight size={17} /></button>
      ...
      <section className="destinations" aria-label="小镇目的地"><div className="destination-intro"><Flag size={17} /><div><span>每一站，都有新发现</span><span className="pixel">THE ADVENTURE IS YOURS.</span></div></div>{[{ id: 'about', number: '01', name: '我的小屋', detail: '关于我和我的故事', icon: House }, { id: 'projects', number: '02', name: '作品研究所', detail: '一些想法，一些创造', icon: Sparkles }, { id: 'contact', number: '03', name: '小镇邮箱', detail: '让新的故事从这里开始', icon: Mail }].map(place => <button key={place.id} className={`destination destination-${place.id}`} onClick={() => open(place.id as Destination)}><span className="destination-icon"><place.icon size={22} strokeWidth={1.5} /></span><span className="destination-text"><span><small className="pixel">{place.number}</small>{place.name}{visited.includes(place.id) && <Check size={12} />}</span><span>{place.detail}</span></span><ArrowUpRight size={17} /></button>)}</section>
    ...
    {modal === 'projects' && <Dialog title="作品研究所" eyebrow="02 / THE LITTLE LABORATORY" onClose={close} wide><Projects /></Dialog>}
